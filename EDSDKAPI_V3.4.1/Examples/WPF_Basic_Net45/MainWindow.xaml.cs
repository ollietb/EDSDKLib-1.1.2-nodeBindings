...
namespace WPF_Basic_Net45
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private async void SaveToComputerChBox_Checked(object sender, RoutedEventArgs e)
        {
            try
            {
        ...
        {
            try { Dispatcher.Invoke(delegate { GetCamera(); }); }
            catch (Exception ex) { ShowError(ex); }
        }

        private void ErrorHandler_SevereErrorHappened(object sender, Exception ex)
        {
            Dispatcher.Invoke(delegate { SetUI(false); });
